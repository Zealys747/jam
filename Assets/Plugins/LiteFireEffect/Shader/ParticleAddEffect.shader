Shader "LiteParticleEffect/Effect Add"
{
    Properties
    {
        ...
        // 👇 НОВОЕ
        _PixelSize ("Pixel Size", Range(1, 128)) = 16.0
    }
    SubShader 
    {
        ...
        Pass
        {
            ...
            half4 frag(v2f i) : SV_Target
            {
                ...
                i.color.a *= fade;
#endif
                // 👇 ПИКСЕЛИЗАЦИЯ — округляем UV до ближайшего пикселя
                float2 pixUV = floor(i.uv_MainTex * _PixelSize) / _PixelSize;
                float2 pixCutUV = floor(i.uv_CutOut * _PixelSize) / _PixelSize;

                fixed4 c = tex2D(_MainTex, pixUV); // 👈 было i.uv_MainTex
                c *= i.color;

                fixed ca = tex2D(_CutTex, pixCutUV).a; // 👈 было i.uv_CutOut
                c.a *= ca;
